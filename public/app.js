...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function dragDrop() {
       ...
       let selectedShipIndex = parseInt(selectedShipNameWithIndex.substr(-1))
        ...
        let shipFirstIdVert = parseInt(this.dataset.id) - selectedShipIndex*10;
        let shipLastIdVert = shipFirstIdVert + lastShipIndex*10;
        ...
        else if (!isHorizontal && !newNotAllowedVertical.includes(shipFirstIdVert) && !shipPlaced.includes(shipLastIdVert) && !shipPlaced.includes(shipFirstIdVert)) {
            for (let i=0; i < draggedShipLength; i++) {
                let directionClass
                if (i === 0) directionClass = 'start'
                if (i === draggedShipLength - 1) directionClass = 'end'
...
//         }
    ...
    function enemyTurn(square) {
        ...
        if(userSquares[square].classList.contains('boom') || userSquares[square].classList.contains('miss')){
                 if(gameMode==="singlePlayer")  return playGameSingle();
        }
        else{
